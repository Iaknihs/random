...
0100110
I can choose to remove cards 1, 4, or 5 since these are face up.
If I remove card 1, the game looks like this (using . to signify an empty spot):

1.10110
I had to flip cards 0 and 2 since they were adjacent.
Next I could choose to remove cards 0, 2, 4, or 5. I choose card 0:

..10110
Since it has no adjacent cards, there were no cards to flip.
I can win this game by continuing with: 2, 3, 5, 4, 6.
...
Output Description
Your program must print a sequence of moves that leads to a win.
If there is no solution, it must print "no solution".
...
def solution(cards):
    ...
    def recursion(crds, moves=None, steps=None):
        ...
        if all(char == '.' for char in crds):
            print("ALL COMPLETE")
            print(moves)
            for step in steps:
                print(step)
            return True
        for idx in range(len(crds)):
            if crds[idx] == '1':
                crds[idx] = '.'
                if idx > 0:
                    if crds[idx-1] in FACES:
                        crds[idx-1] = FACES[FACES.index(crds[idx-1])-1]
                if idx < len(crds)-1:
                    if crds[idx+1] in FACES:
                        crds[idx+1] = FACES[FACES.index(crds[idx+1])-1]
                moves.append(idx)
                steps.append(crds[:])
                if recursion(crds[:], moves[:], steps[:]):
                    return True
        return False

    if not recursion(list(str(cards))):
        print("NO SOLUTION")
...
def challenge_inputs():
    ...
    solution("1101110110000001010111011100110")
